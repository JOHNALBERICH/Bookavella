...
import { loginSchema, LoginFormValues } from '@/features/auth/lib/schemas';
...
export default function LoginPage() {
  ...
  const form = useForm<LoginFormValues>({
    ...
    mode: 'onBlur', // ◄ UX AUDIT: Validação inline disparada na perda de foco
    defaultValues: {
      email: '',
      password: '',
    },
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center bg-background px-4 py-12 font-body text-text-primary">
        ...
        <CardContent className="space-y-6">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              {/* Submit Button com Spinner de Mutação */}
              <Button
                type="submit"
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-surface px-2 text-text-secondary text-[10px] tracking-widest font-semibold uppercase">or join us</span>
              </div>
            </div>

            <div className="text-center space-y-2.5 text-xs text-text-secondary">
              <p>
                Don't have an account?{' '}
                <Link to="/auth/register" className="text-accent hover:text-accent-hover font-semibold transition-colors">
                  Register
                </Link>
              </p>
              <p>
                Are you a property owner?{' '}
                <Link to="/auth/owner/register" className="text-accent hover:text-accent-hover font-semibold transition-colors">
                  Register as Owner
                </Link>
              </p>
            </div>
